...
>
> * 思路
>   * 在父进程接受客户端请求时，创建出一个新进程来为新客户端提供服务
> * 处理过程
>   * 有新请求时就`fork`出一个进程
>   * 注意子进程和父进程的监听描述符和连接描述符那个应该进行关闭
>   * 让子进程处理之前的一个请求，而父进程可以继续进行监听
> * 处理僵尸进程
...
>   * 但这种方式编码复杂
>   * 随着并发粒度的减小，代码复杂度还会显著上升。此种情况下的并发粒度就是读一个完整行所需要的指令数量
>
> ### 基于线程的并发
>
> * 特点
>
>   * 线程方式是上面两种方式的综合
>   * 线程有自己的上下文，包括线程ID、栈、栈指针、程序计数器、通用目的寄存器、条件码
>   * 即线程的上下文已经有很多东西了
>   * 同一个进程的所有线程共享虚拟地址空间
...
>   ```
>
>   * 创建线程，在新线程上下文中运行`线程例程f`，返回线程号
>   * 可以使用`pthread_self`函数或许当前线程的线程号
>
> * 终止线程
>
>   * 当顶层的线程例程返回时，线程隐式终止
>   * 当调用`pthread_exit`函数，线程显示终止。如果主线程调用，会等待所有其他线程终止
...
>     * 可结合的
>     * 分离的
>   * 可结合线程能被其他线程回收或者杀死。除非被其他线程回收，否则资源不会释放
>   * 分离的线程是不能被其他线程回收或者杀死，它的内存资源在它终止时由系统自动收回
>   * 默认情况线程是`可结合的`，因此为了避免内存泄漏，每个可结合线程要么被其他线程显示回收，要么调用`pthread_detach`函数来变成分离的
>   * 使用分离线程例子：对于每个线程单独处理一个请求的情况，因为单独处理就没必要等待每个线程终止，因此在每个子线程开始处理请求前要使用`pthread_detach`变成分离线程
>
> * 基于线程的并发服务器
>
>   * 整体思路和基于进程的并发服务器类似
>   * 如何向新进程传递已连接描述符？最好办法是传递一个指向这个描述符的指针
>
> * 线程的内存模型
>
...
>
> * 用的一种叫做`Java监控器`
> * 它提供了对信号量互斥和调度能力的更高级别抽象
>
> ### 基于线程的事件驱动程序
>
> * IO多路复用不是编写事件驱动程序的唯一方法
