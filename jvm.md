...
* 三种编译器
  * 前端编译器
    * 指的就是传统`C编译器`的前端部分
    * 如`javac`，把`.java`文件编译成`.class`文件
  * 即时编译器`JITC`
    * 在`JVM`中将`class字节码`直接生成物理机能够执行的本地代码
    * 如`HotSpot`的C1和C2编译器，`Graal`编译器
  * 静态提前编译器`AOT`
...
* `javac`前端编译器
  * 无程序优化
    * `javac`几乎无程序优化
    * 优化放到`JVM中`的即时编译器进行，因为这样非java但运行在JVM上的程序依然能享受优化红利
  * 语法糖
    * 语法糖几乎全都是靠前端编译器`javac`去实现的
    * 相当多的新生`java`特性，都是靠`javac`来实现的，而不是依赖字节码或者底层虚拟机的支持
...
* 语言实现
  * `HotSpot`虚拟机全部由C++(少量C)实现
  * `javac`是百分之百`java`语言编写实现的一个程序

* 位置
  * `javac`的源码就在JDK源码中

* 宽松的JVM规范
  * JVM对`class`格式做了非常详尽的约束
  * 然而对`class`文件由何而来几乎没什么约束
...
* 作用
  * 实际可以看作是一组`javac`插件，当插件工作时，允许读取、修改、添加AST中的任意元素
* 递归过程
  * 一旦注解处理器解析出新的符号，就要返回到填充符号阶段重新处理
* 大的访问范围
  * 注解处理器能访问`AST`中的任意元素，包括代码注释能够访问到
* 结果
  * 插入式注解处理器实现的插件在功能上有很大的发挥空间
  * 产生了很多实用的工具如`Lombok`
...
* 语义的正确性
  * AST不能保证语义的正确性
  * 比如 `bool b = 3;`符合`AST`语法但不符合语义

* 任务
  * 对结构上正确(即符合AST语法)的程序进行上下文相关性质的检查
  * 如类型检查、控制流分析、数据流检查
...
* 过程
  ...
  * 数据、控制流分析
    ...
    * 运行期与编译期的数据流检查
      ...
      * 比如局部变量的`final`符号，在运行时`class`字节码内根本没有`final`符号的相关表示，这是一个只能在编译期进行检测的`case`
  * 解语法糖
    * 将语法还原为基础的语法结构
    * 如自动装箱、泛型、变长参数
...
* 任务
  * 将语法树，符号表等信息转换为字节码指令写到磁盘中
  * 并进行少量的代码添加和转换工作

* 代码添加举例
  * 在填充符号表阶段添加实例默认构造函数`<init>()`
  * 将`static{}`块操作收敛到`<clinit>()`内
  * 将字符串加法替换为`StringBuffer`或`StringBuilder`
* 最后再生成字节码
