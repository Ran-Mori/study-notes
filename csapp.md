...
* 高级别IO
  * ANSI提供的标准库，比如`printf, scanf`等
  * C++中重载的`<<`和`>>`
* 低级别IO
  * Linux系统内核提供的系统级Unix IO
* 关系
  * 高级别IO是由低级别IO来实现的

### 为什么要学习低层次IO

* 了解IO能更好了解其他系统概念。如进程创建和文件循环依赖
* 有时除了UnixIO外别无选择。比如标准高层次IO库无读取文件源数据的方式

### 一致性的IO方式

* 概念
  * 所有东西全部都模型化为文件
  * 这样就允许内核引出一个简单、低级、一致的应用接口，称为Unix IO
* 打开文件
...
* 普通文件
  * 包括文本文件、二进制文件、数据格式文件、压缩文件等，但内核不区分这些文件的种类，都是普通文件
  * 文本文件的换行符是`\n`，一个ASCII码
* 目录文件
  * 目录文件实际上是包含一组链接的文件
...
* 套接字文件
  * 用来与另一个进程进行跨网络通信的文件
  * 套接字文件系统是一个用户不可见的，高度简化的，用于汇集网络套接字的内存文件系统，它没有块设备， 没有子目录，没有文件缓冲，它借用虚拟文件系统的框架来使套接字与文件描述字具有相同的用户接口。当用户用socket(family,type,protocol)创建一个套接字时, 系统就在套接字文件系统中为其创建了一个名称为其索引节点编号的套接字文件
* 块设备文件
  * 以一个字块为单位进行存取
  * 普通文件的处理是不必要对硬件进行过多操作的，而字符型设备和块设备就不同了，所以是以特别形式文件出现
  * 用得最多的地方就是硬盘，比如`/dev/had1`
* 字符设备
  * 按字节来访问，还是像块设备一样，存取与硬件精密相关
  * 字符驱动则负责驱动字符设备，这样的驱动通常实现open、close、read和write系统调用。例如：串口、Led、按键等
  * 通过字符设备文件如`/dev/audio(说明audio是字节流)`，应用程序可以使用相应的字符设备驱动来控制字符设备
* 管道文件
  * 用于进程间通信
* 链接文件
  * 就是链接
...
* 写文件
  ...
  * 不足值处理
    * 读和写都可能会遇到不足
    * 比如读遇到EOF，从终端读取文本行，网络延迟读取套接字等时候
    * 对于想创建稳健的web服务器，必须反复读或写来处理好不足值，直到所有需要传送的字节都传送完毕
...
* 是对`Unix IO`函数的一个封装
* 它会自动处理`不足值`，特别适合容易出现不足值的网络程序中使用
...
* 内核缓冲
  ...
  * 数据流向：`数据 -> 内核缓冲区 -> 磁盘`
* 应用缓冲
  * 即我们使用高级语言时某些接口带缓冲，某些接口不带缓冲
  * 有缓冲的目的是为了减少`系统调用`的次数，因为系统调用是一个费时操作
  * 数据流向：`数据 -> 应用级缓冲区 -> 内核缓冲区 -> 磁盘`
* 无缓冲函数使用场景
  * 在网络与内存之间传递二进制数据
...
* `fork`共享文件
  ...
  * 等于就是把指针复制了一份
  * 此时子或父进程关闭文件不是所有都关闭，等于只是少了一个指针引用。因此若想全部关闭文件(内核回收清楚文件资源)则需要子和父进程都关闭对应的文件描述符

### 重定向

* 概念
  * 允许用户将标准输入输出与磁盘文件联系起来
  * 可以通过`int dup2(int oldfd, int newfd)`函数来实现
* `dup(4,1)`
  * 即用描述表项1(标准输出)的内容覆盖描述符项4的内容
  * 调用`dup(4,1)`即将`文件描述符1`指向的文件表项由原来的`文件表项1`变成了`文件表项4`
  * 即重定向根本没有复制再粘贴的过程，而是直接重定向了

### 标准IO

* C语言定义了一套标准IO，称为标准IO库，是为程序员提供了Unix IO的高层次替代
* 如`fopen、fread、fgets、scanf`
* 标准IO库将一个打开的文件`模型化`为一个`流`。类型为`File`的流是对`文件描述符和应用级流缓冲区`的一个高级抽象
...
* 标准IO流是`全双工的`，即能够同时读和写。然而对流的限制和对套接字的限制，有时会相互冲突有限制
* 限制情况
  * 跟在读函数之后的写函数。中间必须有`清空缓冲区`或者`重置当前文件位置k`的操作
  * 跟在写函数之后的读函数。中间必须有`重置当前文件位置k`的操作
* 解决方式
  * 情况一：每次写时都刷新缓冲区
  * 情况二：每次都同时打开两个流，一个用于读，一个用于写
    * 但这种方法需要关闭两个文件描述符来避免内存泄漏
* 套接字文件
  * 对套接字文件使用`seek`是非法的
  * 基于以上种种原因，对套接字文件不要使用`Unix IO`，而使用封装好了的`RIO`包函数
...
* 一个网络应用都是由一个服务器进程和一个或者多个客户端进程组成的。注意核心单位是**进程**

### 网络

* 对主机而言，网络只是一种IO设备(网络适配器或者称为网卡)，是数据源和数据的接收方
* 对应用程序而言，网络只是一个文件描述符(套接字文件)
* 数据在网络和内存之间来回进行复制
...
* 每台主机都运行着`tcp/ip`协议的软件
* 网络程序的客户端和服务端使用`套接字函数`和`unix io`进行通信
* 通常将套接字函数实现为系统调用，这些系统调用会陷入内核态，在内核态调用`tcp/ip函数`

### `tcp/ip`

* `tcp/ip`是一个协议族，每个协议都提供了不同的功能
* `ip`协议提供了最基本的`命名方法`和`递送机制`，但`ip`协议是不可靠的
* `udp`稍微拓展了一下`ip`，也是不可靠的
* `tcp`是构建在`ip`之上的一个复杂协议，提供进程间`可靠的全双工通信`
* `tcp/ip`协议规定了网络字节顺序(即大小端)

### 域名映射

* 1988年之前域名映射通过`HOSTS.TXT`的文本文件手工维护
* 之后通过分布世界范围内的数据库系统来维护
* 多个域名可以映射到同一个ip地址，多个ip地址也可以映射到同一个域名
...
* 连接的端点
  * 一个套接字
  * 客户端的套接字端口是内核自动分配的，而服务端的端口通常是著名端口
...
* 结构
  ...
  * 客户端
    * `open_clientfd`
      * `getaddinfo`
      * `socket`
      * `connet`（发起连接请求，对应服务端`accept`）
    * `rio_writen`（向服务端发送内容，对应服务端`rio_readlined`）
    * `rio_readlined`（客户端接收服务端数据完毕，即将准备关闭流，对应服务端`rio_writen`）
    * `close`(发出一个EOF，对应服务端`rio_readlined`)
  ...
  * 服务端
    * `open_listenfd`
      ...
      * `bind`
      * `listen`
    * `accept`（接收客户端请求，对应客户端`connect`）
    * `rio_readlined`（读取客户端发送内容，对应客户端`rio_writen`）
    * `rio_writen`（向客户端发送内容，对饮客户端`rio_readlined`）
    * `rio_readlined`（读遇到EOF，对应客户端`close`）
    * `close`（客户端关闭连接后，服务端也关闭）

* 概述

  * 是一组函数，它们和`Unix IO`结合起来，用以创建网络应用
  * 是一套编程接口，具体实现由操作系统内核完成，我们只管调用
  * 市面上的操作系统都实现了套接字接口，如`windows、Linux、Macintosh`
  * 从内核来看，一个套接字是通信的一个端点；从程序角度看，套接字是一个具有相应描述符的打开文件
...
* accept函数
  ...
  * 等待来自客户端的连接请求到达`监听描述符listenfd`，然后再addr中填写客户端的套接字地址，返回一个`已连接套接字描述符`，这个描述符可被用来利用Unix IO函数与客户端通信
  * 监听描述符
    * 是作为客户端连接请求的一个端点
    * 被创建一次
    * 存在于服务器的整个生命周期
...
* 服务端调用`lisnten`，即有了一个`listenfd`
* 客户端调用`connect`，即有了一个`clientfd`。`connect`函数是阻塞的
* 服务端监听到一个连接，于是双端建立一个连接。服务端会有一个新的已连接描述符`connfd`
* 此时`connect`阻塞函数返回，`clientfd-connfd`组成连接套接字对，就可以使用`unix io`进行通信了
* 最后`lisntenfd`保持不变，继续监听连接

### 为什么区分连接套接字和监听套接字

* 乍一看是复杂了
* 然而这样有利于建立并发服务器，同时处理和多个客户端的连接

### 主机和服务的转换

* `getaddrinfo, getnameinfo`实现二进制套接字地址结构的主机名、主机地址、服务名和端口号的字符串表示之间的互相转化
* 当和套接字函数一起使用时，能编写出独立于任何特定版本的IP协议的网络程序。即我们在套接字函数要使用地址时，都使用`getaddrinfo`这类函数，就会与协议无关
...
* 并发不仅仅局限于操作系统内核，如内核并发控制多个应用程序
* 应用程序级别的并发也十分重要，如Linux信号机制响应异步事件

### 应用级并发其他用处

* 访问慢速设备时，让出CPU。让整个系统更高效的执行。
  * 注意这个让出的过程是应用程序自己控制的，这就是应用级别的并发而不是操作系统内核级别的并发
* 与人交互。一个应用程序至少要能同时响应多种操作，这就是并发
  * 即每一个操作是单独的逻辑流
* 通过推迟工作来降低延迟。通过推迟其他操作和并发地执行它们来降低延迟
* 建立并发服务器来服务多个网络客户端。为每个客户端单独创建一个逻辑流
* 在多核机器上进行并行计算

### 构造并发的三种方法

* 进程
  * 每个逻辑控制流都是一个进程，由内核来完成调度和维护
  * 进程虚拟地址空间彼此隔离，如果想要通信就要通过IPC
* IO多路复用
  * 在一个进程中显示地由用户自己调度它自己的逻辑流
  * 逻辑流常被模型化为状态机，逻辑流的变化就是状态在的变化
  * 只有唯一一个进程，因此地址空间是所有逻辑流共享的
* 线程
  * 是运行在单一进程中上下文中的逻辑流，由内核进行调度
  * 它综合了线程由内核调度，IO多路复用共享地址空间的特点

### 基于进程构建并发

* 思路
  * 在父进程接受客户端请求时，创建出一个新的子进程来为新客户端提供服务，父进程继续监听
* 处理过程
  * 有新请求时就`fork`出一个进程
  * 子进程关闭监听描述符，父进程关闭已连接描述符。不然可能内存泄漏
  * 让子进程处理新的请求，而父进程可以继续进行监听
* 处理僵尸进程
  * 通常服务器会运行很久，所以我们必须包含一个`SIGCHLD`处理程序来回收僵尸进程的资源
* 进程的特点
  * 父子共享文件表但不共享虚拟空间地址
  * 如果通信必须通过IPC机制
...
* 思路
  * 使用`select`函数，要求内核挂起进程，只有一个或者多个IO事件发生后，才将控制权返回给应用程序
  * 即使用同一个进程，但自己处理各种情况，相当于是用户程序自己做事件调度
  * 
* select函数使用场景之一
  * 等待一组描述符准备好读
* IO多路复用与并发事件驱动服务器
  * IO多路复用可以用做并发事件驱动程序的基础
  * 一般会把事件流(逻辑流)抽象成状态机
* 事件驱动的web服务器
  * 这种服务器虽然缺点很多，但现代高性能web服务器(Node.js、nginx)全部都采取这种方式
  * 采用的是基于IO多路复用的事件驱动的编程方式
  * 主要是相比于进程和线程，这种方式有明显的性能优势
* 优劣势
  * 相比于进程，它给程序员更多对程序行为的控制
  * 运行在单进程中，所有逻辑流共享一个虚拟地址内存空间，甚至可以使用GDB像对普通程序一样进行调试
  * 但这种方式编码复杂
  * 随着并发粒度的减小，代码复杂度还会显著上升,比如状态机状态数指数增加。此种情况下的并发粒度就是读一个完整行所需要的指令数量

### 基于线程的并发

* 线程方式是上面两种方式的综合

* 线程有自己的上下文，包括线程ID、栈、栈指针、程序计数器、通用目的寄存器、条件码。即线程的上下文已经有很多东西了
* 同一个进程的所有线程共享虚拟地址空间
* 线程由内核进行调度，程序员不用负责调用线程

### 主线程与其他线程

* 主线程与其他线程的唯一区别就是它是进程中运行的第一个线程

* 对等线程池

  * 概念：和一个进程相关的所有线程组成一个对等线程池
...
* 概念
  * 是C程序中处理线程的一个标准接口，包含了大约60多个操作线程的函数
* 作用
  * 能够创建、杀死、回收、与对等进程安全共享数据、通知对等线程变化
...
* 创建线程
  ...
  * 创建线程，在新线程上下文中运行`线程例程f`。

  * `tid`引用是会返回的线程ID

  * 可以使用`pthread_self`函数或许当前线程的线程ID
...
* 终止线程
  ...
  * 当顶层的`线程例程`返回时，线程隐式终止

  * 当调用`pthread_exit`函数，线程显示终止。如果主线程调用`pthread_exit`，会等待所有其他线程终止

    ```c
    void pthread_exit(void *thread_return);
    ```

  * 对某个线程调用`linux`的`exit`函数，会终止进程及所有相关的线程

  * 对等线程通过传入此线程ID作为参数调用`pthread_cancel`来终止此线程
...
* 回收已终止的线程资源
  ...
  * 线程通过调用`phtead_join`来等待其他线程终止
  * 阻塞直到`tid`线程终止，然后回收终止线程所有的内存资源
  * 和Linux的`wait`有些不同
...
* 线程有两种状态
  ...
  * 可结合的 —— 能被其他线程回收或者杀死。除非被其他线程回收，否则资源不会释放
  * 分离的 —— 不能被其他线程回收或者杀死，它的内存资源在它终止时由系统自动收回

* 默认情况

  * 线程是`可结合的`。

  * 因此为了避免内存泄漏，每个可结合线程要么被其他线程显示回收，要么调用`pthread_detach`函数来变成可分离的
...
* 使用分离线程例子
  ...
  * 对于每个线程单独处理一个请求的情况，因为单独处理就没必要等待每个线程终止，因此在每个子线程开始处理请求前要使用`pthread_detach`变成分离线程。这样处理完成时线程例程执行完毕，资源自动回收
...
* 如何向新进程传递已连接描述符
  ...
  * 最好办法是传递一个指向这个描述符的指针
  ...
  ```c
  //创建线程
  pthead_create(&tid, NULL, thread, &connfd);
  
  //线程例程内
  int connfd = *((int *)vargp);//保存为一个局部变量
  ```

* 注意如果不回收线程资源就要让线程是可分离的

### 线程的内存模型

* 每个线程有自己独立的线程上下文，包括线程ID、栈、栈指针、程序计数器、通用目的寄存器、条件码
* 每个线程和其他线程一起共享进程上下文的其他部分
* 线程们共享相同的打开文件集合
* 线程寄存器是不共享的，而虚拟内存是共享的
* 线程的栈空间被保存在虚拟地址空间的栈区域中，并且通常是自己线程访问自己的栈

### 将变量映射到内存

* 全局变量。虚拟地址空间中只有一个实例，所有线程都能访问
* 本地自动变量，即定义在函数内部且无static修饰。每个线程运行到此函数时都有一个此变量的单独实例
* 本地静态变量，即定义在函数内部且有static修饰。虚拟地址空间中只有一个实例
...
* 线程同步信号量
  ...
  * 是一个非负整数值全局变量，只能用两个特殊的操作`P、V`来操作这个变量

  * P操作

    * 如果信号量非零，就减少1并立即返回。如果是零，就挂起线程并轮
  ...
  * POSIX定义了很多操作信号量的函数
    ...
    ```c
    int sem_init(sem_t *sem, unsigend int value);//初始化信号量
...
* 用的一种叫做`Java监控器`
* 它提供了对`信号量互斥和调度能力`的`更高级别抽象`

### 基于线程的事件驱动程序

* IO多路复用并不是编写事件驱动程序的唯一方法
* 比如示例是一个带有主线程和工作线程的状态机，主线程有多个状态，每个工作线程有一个状态
* 它们依然可以构成一个状态机，执行状态的转换

### 使用线程提高并行性

* 就是在多核或者超线程处理器上运行
* 并行程序是并发程序的一个子集
* 多线程不一定运行得更快，因为它有多线程的`同步开销`

### 线程安全

* 四个不相交的线程不安全函数类
  * 不保护共享变量的函数
  * 保持跨越多个调用的状态的函数。比如多线程调用`rand`函数因为随机数种子的问题而导致和单线程调用有差别
  * 返回指向静态变量和指针的函数
  * 调用线程不安全函数的函数
* 解决上面四种函数的方法
  * 重写
  * 加锁
* 可重入性
  * 可重入函数只是线程安全函数的一个子集，并不是只有可重入函数是线程安全的
  * 特点：不会引用任何共享数据
  * 显示可重入：函数参数全是值传递，并且函数体只引用栈变量(不引用全局变量)
  * 隐式可重入：在显示可重入的基础上放松要求，允许部分参数使用引用传递
* 库函数
  * 大部分库函数都是线程安全的
* 加锁的缺点
  * 线程同步降低程序运行的速度
  * 当返回复杂结构的指针时，要深层复制整个结构，十分浪费时间
  * 对于`rand`这种函数加锁都没有效果
