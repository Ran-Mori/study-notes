...
* ASIC resistance。BTC的PoW的統計方式是計算hash，導致目前僅僅靠普通的電腦根本無法挖礦，要靠專門的ASIC芯片才能挖礦。這於最初的去中心化思路相違背，因此預期要設計一個新的挖礦算法(PoW)，能夠ASIC resistance
* difficult to resolve, easy to verify. 因此挖礦算法要驗證簡單

### 设计

* 當前ASIC都是計算密集型，因此要設計一個內存密集型算法。 Memory Hard Puzzle.
* 為了要difficult to resolve, easy to verify。不能驗證的時候也需要很大的內存。
* 因此設計了兩個內存，小的大小是16M，大的是1GB。過程也是嘗試各種nonce，通過計算hash填充數字來挖礦。
* 計算機的內存隨著時間也在增長，因此ETH也會調整挖礦難度，16M和1GB的大小也不是固定不變的。

### 其他

* 然後ETH一直在嚇唬，說未來不採用挖礦的方式了，要從PoW轉向PoS(stake)。由於設計投產一款ASIC芯片需要的週期長，前期投入成本高，加上ETH一直在恐嚇馬上就不用挖礦了。也是一種變相的ASIC resistance.
* ETH給開發者原始人員預留了很多幣。很多時候你再努力挖再多幣不如當創始成員一開始分的幣多。而BTC則沒有預留幣的機制，它只是一開始挖礦難度很低而已。
* BTC每2016個區塊調整一次挖礦難度，大概是2週。ETH預期每15s出一個區塊，因此每一個區塊都會調整挖礦難度。具體算法就是根據前一個區塊的出塊時間和15s相比，如果大於15s就降低挖礦難度，如果小於15s就增加挖礦難度。
...
* ETH的核心團隊為了解決the dao智能合約的bug，採取硬分叉的方式來升級軟件協議版本，想要回滾交易。原教旨主義的一隊人認為這簡直是危言聳聽，這根本就不是去中心化的貨幣，而且比中心化的操作方式還爛。因此不認可新的協議，新的鏈，繼續在舊鏈上挖。
* 從此以後新鏈繼續叫ETH，舊鏈叫ETC(ETH Classic)

## thinking

* 不可傳改姓是一柄雙刃劍
* 沒有什麼事不可傳改的
* many eyeball fallacy - 開源的不一定是安全的，人人都認為世界上其他人肯定看過代碼，實際上可能根本就沒什麼人看過
* 去中心化不是說規則不能改，而是改規則也要用去中心化的方式。the dao事件的核心就是多數礦工支持升級新的版本軟件
* 分叉不一定是壞事，而分叉恰好是去中心化的一種體現
* 去中心化一定運行在分布式上；運行在分布式上不一定去中心化。一般的去中心化是不同節點運行不同的邏輯，最後匯總得到結果；而去中心化用的分佈式是所有的節點其實執行的是同一件事情，保證共識的一致
* 現有支付手段已經能解決的場景其實使用加密貨幣的必要性不大，比如說咖啡店支持比特幣。往往是跨國轉賬，銀行不互通等場景才是使用比特幣的最佳場景。
